...
Log–log, both axes nodes explored, dashed `y = x`. One dot = one (presentation, rewrite) pair where **both** the plain run and the rewritten run finished inside the 10,000-node budget: 60 presentations × 45 CoVs, 546 such pairs. Orange above the line, blue below. **Step 1** overlays the black diamonds — for each presentation, the cheapest CoV that solved it. The denominator is 40, not 60: those are the presentations where plain greedy *and* at least one CoV both finished in budget. Split is 37 fewer nodes / 1 equal / 2 more. The nine rescues (greedy failed, a CoV solved) are deliberately **not** on the slide — say them yourself. The slide says CoV throughout; say "change of variables" in full the first time, then CoV.
...
- **"Does abelian mass always work?" / "Where does it fail?"** → appendix a8. Three uses, one works. It **ranks** a presentation's own CoV candidates well (ρ +0.81 within-presentation, AUC 0.922, positive on 52/52). It does **not screen** between presentations — pooled over all 6,722 rows it reads r +0.11 while length reads +0.46, and the pooled number is the one that would answer "is this presentation worth a CoV at all"; that reversal is Simpson's paradox, not noise, and it reproduces at every budget. And it does **not pick** — mass is a small integer, a median of 6 candidates tie at each minimum, so one search gets 47.4/60 in expectation over random tie-breaks, not the 52/60 the oracle set reaches; best-of-3 recovers it. The honest one-liner: it shortlists, it does not pick, and it never says whether to bother.
